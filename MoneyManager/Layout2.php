...
include 'conn.php';
  $BPID = $_GET['BPID'];
  $BudgetPlanTitleSQL = "SELECT BudgetTitle FROM budgetplan WHERE BudgetPlanID = '$BPID'; ";
  $BudgetPlanResult = mysqli_query($conn, $BudgetPlanTitleSQL);
  if (mysqli_num_rows($BudgetPlanResult)<=0) {
    echo "<script>alert('No budget title information, technical errors!');</script>";
  }else {
    $BudgetPlanRows = mysqli_fetch_array($BudgetPlanResult);
    $BudgetPlanTitle = $BudgetPlanRows['BudgetTitle'];
  }
...
?>
  ...
  <body class="bg">
      ...
      <div class="navbar navbar-expand-lg navbar-dark bg-dark navbar-custom">
        ...
        <!-- Search form -->
        <h3 class="ml-auto col-sm-9 col text-white text-center justify-content-center" ><?php echo $BudgetPlanTitle;?></h3>
