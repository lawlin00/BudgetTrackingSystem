...
<?php
  ...
  $buffer=str_replace("%title%","UserHome",$buffer);
  echo $buffer;
  include 'Layout.php';
  $searchkey= $_POST['searchkey'];

  if (empty($searchkey)) {
    echo "<script>alert('Please enter search key');</script>";
...
<div class="row-fluid">
  <div class="col-md-12">
    <?php
      include 'conn.php';
      $sql = "SELECT * FROM budgetplan WHERE TeamID IS NULL AND UserID = '$S_UserID' AND BudgetPlanStatus ='0' AND BudgetTitle Like '%$searchkey%' order by BudgetTitle asc;";
      $result = mysqli_query($conn,$sql);

      if (mysqli_num_rows($result)<=0){
      //  echo "<script>alert('No Personal Budget Plan');</script>";
      //  echo "<script>window.location.href = 'NewBudgetPlan.php';</script>";
      echo "<div class='d-flex justify-content-md-center mt-3 mb-5'><button class='btn btn-primary'>Create New Budget Plan</button></div>";
...
<?php
  ...
  if (mysqli_num_rows($TeamResult)<=0){
    ...
    echo "<div class='d-flex justify-content-md-center mt-3 mb-5'><button class='btn btn-primary'>Create Team</button></div>";
    echo "</div>";
  }else {
    while ($TeamRows = mysqli_fetch_array($TeamResult)) {
      ...
      $BudgetPlanSQL = "SELECT * FROM budgetplan WHERE TeamID = '$TeamID'AND BudgetPlanStatus ='0'  AND BudgetTitle Like '%$searchkey%' order by BudgetTitle asc;";
